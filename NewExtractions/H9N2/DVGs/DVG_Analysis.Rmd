...
seg_rich = merge(seg_rich, temp, by = c('ferret_id', 'dpi', 'inf_route', 'diet', 
           'pair_numbers', 'pair_diets', 'type_day', 'segment'), all = TRUE) 
...
for(i in samples){
  ...
  if(nrow(d) > 0){
    ...
    comp = merge(n, d, by = c("pair_diets","pair_numbers","pair","segment","seg_dvg","DVG_group","GroupBoundaries"), all.x = TRUE) %>%
      count(pair_numbers,ferret_day.x, ferret_day.y)
