...
metafile = glue("{wkdir}/scripts/UPDATED.H9N2.metadata.csv")
...
```{r}
temp = meta %>% 
        filter(inf_route %in% c('Index','Contact')) %>% 
        unique() %>% 
...
Sp20_shared = Sp20_index %>% filter(DVG %in% Sp20_stock_dvg) %>% filter((DVG %in% Sp20_index_dvg)) %>% unique()
Sp20_denovo = Sp20_index %>% filter((DVG %in% Sp20_index_dvg)) %>% filter(!(DVG %in% Sp20_stock_dvg)) %>% unique()
...
```{r}
index_obese = filter(index, ferret_weight == "obese") 
o_dvg = unique(index_obese$DVG)

index_lean = filter(index, ferret_weight == "lean") 
