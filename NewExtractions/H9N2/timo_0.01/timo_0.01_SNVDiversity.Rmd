...
samples = unique(af_df$pairs)
  
ferret_files_dir = "~/Desktop/GitHub/Obesity/NewExtractions/H9N2/timo_0.01/freqfiles/transmission"
  
for(i in samples){
  print(i)
  
  i = filter(af_df, pair == i, inf_route == "Index")
  c = filter(af_df, pair == i, inf_route == "Contact")
  
  comp = merge(i, c, by = c("segment", "ntpos","stocknt","major","minor","cohort","var"), all.x = TRUE)
  comp$minorfreq.y[is.na(comp$minorfreq.y)] = 0
  
  df = select(comp,minorfreq.x,minorfreq.y)
  write.table(df, file = glue("{ferret_files_dir}/{i}_{c}_freqs.csv"), row.names = FALSE, col.names = FALSE)
