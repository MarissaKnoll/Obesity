...
```{r}
freq_dist = ggplot() +
  geom_boxplot(aes(x = cat, y = n, color = diet), filter(minors, inf_route == "Index"), outlier.shape = NA) +
  geom_jitter(aes(x = cat, y = n, color = diet, group = diet), filter(minors, inf_route == "Index"), width = 0.2) +
  geom_boxplot(aes(x = cat, y = n, color = diet), filter(majors, inf_route == "Index"), outlier.shape = NA) +
  geom_jitter(aes(x = cat, y = n, color = diet, group = diet), filter(majors, inf_route == "Index"), width = 0.2) +
  PlotTheme1 +
  DietcolScale
print(freq_dist)
...
```{r}
o = filter(minors, inf_route == "Index" & cat == "50-99.99%" & diet == "Obese")
l = filter(minors, inf_route == "Index" & cat == "50-99.99%" & diet == "Lean")
t.test(o$n,l$n)
...
```{r}
# Is there a bias in terms of dpi in above plot?
...
counts = filter(gen_count,inf_route == "Index") %>% filter(DPI == "d02" | DPI == "d04" | DPI == "d06")
...
m = lm(test_contact$snv_count ~ test_contact$Ct_Mgene)
summary(m)
```
