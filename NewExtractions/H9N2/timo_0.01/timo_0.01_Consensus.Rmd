...
```{r}
con_change$varpos = paste0(con_change$segment,"_",con_change$ntpos)
# using the position to detect when the consensus and minor flip, checked that there were only two nts found for every position
consensus_varpos = unique(con_change$varpos)
consensus_ferrets = unique(con_change$ferretID)
minorvdf$varpos = paste0(minorvdf$segment,"_",minorvdf$ntpos)

minors_before_consensus = filter(minorvdf, varpos %in% consensus_varpos) %>% unique()
minors_before_consensus$freq = minors_before_consensus$minorfreq
minors_before_consensus$aa_change = paste0(minors_before_consensus$stockaa,minors_before_consensus$aapos,minors_before_consensus$majoraa)
minors_before_consensus$nt_change = paste0(minors_before_consensus$stocknt,minors_before_consensus$ntpos,minors_before_consensus$major)

consensus_changes = con_change %>% select(!c("maj")) %>% unique()
consensus_changes$freq = consensus_changes$majorfreq

all_pos = rbind(minors_before_consensus,consensus_changes) %>% unique()
all_pos$segment = factor(all_pos$segment, levels = SEGMENTS)
all_pos$diet = factor(all_pos$diet, levels = c("Control","Obese","Lean"))

all_pos = filter(all_pos, ferretID == "HK1073" | 
                ferretID == "1408"| ferretID == "1409"|
...
#only need the ferrets in which the consensus changes are found

varpos_plot = ggplot(all_pos, aes(x = DPI, y = freq)) + 
  geom_point(aes(color = varpos), size = 0.5) +
  geom_line(aes(group = varpos, color = varpos)) +
  geom_hline(yintercept = 0, linetype = "dotted", color = "black") +
  facet_grid(cohort~ferretID) +
  PlotTheme1
print(varpos_plot)
ggsave("Consensus_Vars_All_Observations.pdf", varpos_plot, path = savedir, height = 5, width = 7)
```
