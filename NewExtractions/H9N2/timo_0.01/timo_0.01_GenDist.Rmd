...
mfit2 = merge(mfit2, meta, by = c('name')) 
...
efit2 = merge(efit2, meta, by = c('name')) 

ggplot() +
    geom_point(data = efit2, aes(x=V1, y= V2,shape = diet, color =inf_route), size = 2) +
    geom_text(data = efit2 %>% filter(diet == 'Control'), 
                aes(x=V1, y= V2,label = name), 
       hjust=0, vjust=0, angle=45) +
    PlotTheme1
```
...
ln_d02 = filter(man_long, inf_route.x == "Control" & inf_route.y == "Index" &
                  cohort.x == cohort.y &
                  diet.y == "Lean" & DPI.y == "d02")
t.test(ob_d02$dist,ln_d0$dist)
```
