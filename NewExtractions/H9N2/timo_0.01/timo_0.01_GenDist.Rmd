...
ggplot() +
    geom_point(data = mfit2, aes(x=V1, y= V2,shape = diet, color =inf_route), size = 2) +
    geom_text(data = mfit2 %>% filter(diet == 'Control'), 
...
interesting = c("1414_d06","Sp20_HK1073","Sp19_HK1073","Sm18_HK1073","F17_HK1073")
  
ggplot() +
  geom_point(data = efit2, aes(x=V1, y= V2,shape = diet, color =inf_route), size = 2) +
  geom_text(data = efit2 %>% filter(name %in% interesting),
            aes(x=V1, y= V2,label = name),
            hjust=0, vjust=0, angle=45) +
...
```{r}
# T tests
ob_d02 = filter(man_long, inf_route.x == "Control" & inf_route.y == "Index" &
                  cohort.x == cohort.y &
                  diet.y == "Obese" & DPI.y == "d02")
ob_d04 = filter(man_long, inf_route.x == "Control" & inf_route.y == "Index" &
                  cohort.x == cohort.y &
                  diet.y == "Obese" & DPI.y == "d04")
t.test(ln_d02$dist,ln_d04$dist)
```
...
```{r}
ggplot(filter(man_long_m, cohort == "W17", comp == "W17_HK1073", inf_route == "Index"), 
       aes(x = DPI, y = dist)) +
  geom_boxplot(aes(group = "diet")) +
  geom_jitter(width = 0.1, aes(color = diet)) +
  PlotTheme1

ggplot(filter(man_long_m, cohort == "F17", comp == "F17_HK1073", inf_route == "Index"), 
       aes(x = DPI, y = dist)) +
  geom_boxplot() +
  geom_jitter(width = 0.1, aes(color = diet)) +
  PlotTheme1

ggplot(filter(man_long_m, cohort == "Sm18", comp == "Sm18_HK1073", inf_route == "Index"), 
       aes(x = DPI, y = dist)) +
  geom_boxplot() +
  geom_jitter(width = 0.1, aes(color = diet)) +
  PlotTheme1
