...
ggsave(DietUnique_InfRoute, filename = "SegmentSNVPlot_DietUnqique.pdf", path = savedir, width = 15, height = 10)


ggplot(filter(dietunique, n > 1, nonsyn == "nonsyn"), 
                             aes(x = ntpos, 
                                 y = factor(segment, levels = c('H9N2_NS','H9N2_MP','H9N2_NA','H9N2_NP','H9N2_HA','H9N2_PA','H9N2_PB1','H9N2_PB2')))) +
  geom_point(aes(color = transmission, size = n)) + 
  ggtitle("Number of samples containing each variant - diet specific") +
  xlab("Nucleotide position") +
  ylab("Segment") +
  theme(legend.key = element_blank(),
