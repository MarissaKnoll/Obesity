...
dietunique2 = rbind(rest, MP_G459A,MP_T444C,MP_G339A)
```

```{r}
DietUnique = ggplot(filter(dietunique, ferret_num == 2), 
                    aes(x = ntpos,
...
persistence = ggplot(filter(dietunique_repeats, nonsyn == "nonsyn" & fer_num == 2), aes(x = DPI, y = minorfreq)) +
  geom_point() +
  geom_line(aes(group = ntvar, color = ntvar)) +
  facet_grid(diet~ferretID)
print(persistence)
...
SharedPlot = ggplot(shared, 
  ...
  geom_text(data = filter(shared, ferret_num > 4, nonsyn == "nonsyn"), aes(label = aavar, vjust = 2, hjust = 0.5)) +
...
```{r}
nonsyns_shared = filter(shared, nonsyn == "nonsyn" & ferret_num > 1) %>% 
  ungroup() %>% 
...
low_shared = filter(nonsyns_shared, n < 11) %>% unique() %>% mutate(cat = "low")
high_shared = filter(nonsyns_shared, n > 11) %>% unique() %>% mutate(cat = "high")
